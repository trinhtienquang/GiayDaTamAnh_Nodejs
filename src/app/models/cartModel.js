...
const CartModel = {
  ...
  addProductToCart: async (userId, productId, name, image, price, quantity, size) => {
    const query = `INSERT INTO tbl_cart (user_id, product_id, sanpham_ten, sanpham_anh, sanpham_gia, quantity, sanpham_size) 
                   VALUES (?, ?, ?, ?, ?, ?, ?) 
  ...
  removeProductFromCart: async (userId, productId, size) => {
    const query = 'DELETE FROM tbl_cart WHERE user_id = ? AND product_id = ? AND sanpham_size = ?';
